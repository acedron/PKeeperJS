...
const create = () => {
  ...
  else if (password != cpassword) errcall('Passwords don\t match!');
  else {
    ipcRenderer.send('info', `Creating user ${username}.`, Error().stack);
    try {
      if (fs.existsSync(`${maindir}/users.xml`)) fs.readFile(`${maindir}/users.xml`, 'utf8', (err, data) => {
        var jsonout = parser.xml2json(data);
        try {
          var users = jsonout["users"]["user"];
          if (users.length == undefined) {
            if (users.name != username) crypter.encrypt(password, 'p3s6v9y$B&E(H+Mb', pass => {
            ...
            if (valid == true) crypter.encrypt(password, 'p3s6v9y$B&E(H+Mb', pass => {
              var nusers = [];
      ...
      else crypter.encrypt(password, 'p3s6v9y$B&E(H+Mb', pass => {
        fs.writeFile(`${maindir}/users.xml`, jsonxml({
          users:[
            {name: "user", attrs:{name: username, pass: pass} }
          ]
